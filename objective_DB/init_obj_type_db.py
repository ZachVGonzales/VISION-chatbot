...
if __name__ == "__main__":
  ...
  columns = df.columns
  columns = f"id INTEGER PRIMARY KEY AUTOINCREMENT, {columns[0]} TEXT NOT NULL, {columns[1]} TEXT NOT NULL"
  create_table_query = f"CREATE TABLE IF NOT EXISTS objectives ({columns})"
  cursor.execute(create_table_query)

  # Insert filtered data into the table
  for row in df.itertuples(index=False, name=None):
    obj = json.dumps(row[0].split())
    obj_type = json.dumps(row[1].split())
    insert_query = f"INSERT INTO objectives ({', '.join([col for col in df.columns])}) VALUES (?, ?)"
    cursor.execute(insert_query, (obj, obj_type))
