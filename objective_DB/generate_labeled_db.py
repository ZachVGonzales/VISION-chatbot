...
def write_objectives(audience, behavior, condition, degree, label: str):
  ...
  try:
    with open("objectives_labeled.csv", "a", newline="") as file:
      writer = csv.writer(file)
      selection = random.sample(possible_exs, 4)
      for example in selection:
        writer.writerow((example[0], example[1][0], example[1][1], example[1][2], example[1][3], cabd, label))
...
def write_null(null_ex):
  try:
    with open("objectives_labeled.csv", "a", newline="") as file:
      writer = csv.writer(file)
      writer.writerow((null_ex, 0, 0, 0, 0, "undefined", "undefined"))
  except Exception as e:
    print("Error", f"Failed to save data: {e}")
...
if __name__ == "__main__":
  # init csv file
  with open("objectives_labeled.csv", "w", newline="") as file:
    writer = csv.writer(file)
  ...
  for i, behavior in enumerate(random.sample(bl_df["behavior"].to_list(), num_b)):
    ...
    for _ in range(num_rand):
      ...
      write_objectives(audience, behavior, condition, degree, bl_df["type"][i])

  for null_ex in null_exs:
    write_null(null_ex)

  for _ in range(math.ceil(len(bl_df["behavior"])*(num_rand/10))):
