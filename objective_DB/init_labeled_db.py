...
if __name__ == "__main__":
  ...
  columns = df.columns
  columns = f"id INTEGER PRIMARY KEY AUTOINCREMENT, {columns[0]} TEXT NOT NULL, {columns[1]} INTEGER NOT NULL, {columns[2]} INTEGER NOT NULL, {columns[3]} INTEGER NOT NULL, {columns[4]} INTEGER NOT NULL, {columns[5]} TEXT NOT NULL, {columns[6]} TEXT NOT NULL"
  ...
  for row in df.itertuples(index=False, name=None):
    ...
    label = json.dumps(row[6].split())
    insert_query = f"INSERT INTO objectives ({', '.join([col for col in df.columns])}) VALUES (?, ?, ?, ?, ?, ?, ?)"
    cursor.execute(insert_query, (in_obj, row[1], row[2], row[3], row[4], new_obj, label))
