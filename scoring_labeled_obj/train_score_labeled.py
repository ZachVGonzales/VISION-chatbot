...
def init_example_table(cursor: sqlite3.Cursor):
  ...
  cursor.execute("SELECT input_objective, audience_score, behavior_score, condition_score, degree_score, label FROM objectives")
  rows = cursor.fetchall()
  for row in rows:
    input_obj, a, b ,c, d, label = row
    label = json.loads(label)
    if len(label) >= 2:
      l_type1 = 1 if label[0] == "Performance" else 0
      l_type2 = 1 if label[1] == "Terminal" else 0
    else:
      l_type1 = 0
      l_type2 = 0
    labels = [a, b, c, d, l_type1, l_type2]
